...
歡迎您進駐閱大安社區擔任總幹事。本手冊整理了社區的數位系統、設施保養、行政財務、安全防災、住戶關係等各面向的工作內容與標準作業流程，作為您到職訓練、日常工作與未來交接時的依據。

社區自第一屆管委會以來追求「**屬社區的資產而非屬個人的習慣**」——也就是不論誰擔任總幹事，社區的管理品質應該維持穩定、文件格式應該一致、廠商關係與住戶資料應該完整保存。本手冊是這個理念的具體實踐，也希望您在工作中持續累積、優化內容，下任接手時能無縫接續。
...
本章節將剖析造成此現象的成因與影響，並確立本手冊存在的必要性。
...
graph LR
    ...
    Sub2[制治模式] -- 依賴手冊SOP --> Stability(品質穩定)
    Instability --> Reform[建立本手冊]
    Reform --> Sub2
...
本手冊的編撰並非僅為了規範行政細節，而是為了達成以下戰略目標：
...
- **依據**：《臺北市雨水下水道相關設施與用戶排水設備審查暨查驗及檢查要點》
- **責任歸屬**：用戶排水設備的管理、維護由社區（下水道用戶）**自行負責**
- **年度義務**：**每年 4 月**自主上傳維護文件（含維護設施平面圖、現況照片）至**臺北市政府市民服務大平臺**供審查單位參考
- **第一次處理（115 年）**：管委會收到公文後啟動首次自主檢查，建議將上傳紀錄、廠商函詢、檢查照片**集中放入 Google Drive 「年度法規遵循 / 流出抑制設施」資料夾**留供日後對照
...
2. **現況照片**—— 按下表分區拍攝
3. **檢查表**（CC-FORM-OSF-001 列印或存 PDF）
...
任一情境發現時，**儘速聯絡管維公司（潤泰維護）或專業抽水機保養廠商檢修**，**並把改善前後的照片都留下**——這在後續查核輔導時是重要佐證。
...
- **2026-05-17 v3** — 第五屆第一次區權會（5/16 召開）後內容補強。新增 7 個區塊：
  ...
  - 第四章 §4.2.4「會議當天時間軸」（mermaid gantt）+ §4.2.5「會議後文書 SOP」（mermaid flowchart）
