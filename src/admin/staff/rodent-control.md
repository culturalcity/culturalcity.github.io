...
| **室外鐵門、大門（地面傾斜或不平）** | **幾乎做不到** | 地面斜、門扇會擺動下沉、縫內會積灰與石礫；金屬板裁得再準，實務上只能做到十幾至二十公釐，且數月後會再跑掉 |
...
| **而且做不到標準** | 地面斜、門扇會擺動下沉、縫內積灰石礫，實務上只能做到十幾至二十公釐，數月後又會跑掉（見 §3.1 可行性表）。**花錢買一個達不到的規格是最糟的支出** |
...
1. **白天活動**（14:59）。鼠類本為夜行性，在開放區域從容於白天覓食，通常代表族群密度已高到競爭把部分個體推到日間，或環境讓牠們完全不受干擾。**這不是單一迷途個體的行為。**
...
- **發現時請從主機匯出原始片段存檔**（USB 或主機的匯出功能），不要用手機翻拍螢幕——翻拍會損失大半資訊，原始檔才看得出品種與行為細節。
