...
底下的章節是原理與判準，**要動手時看這一節就好**。

## 採購（約 1,500～3,000 元，修繕雜支）

| 品項 | 數量 | 單價（估） | 挑選重點 |
|---|---|---|---|
| 防拆式誘餌站 | 3 | 200–500 元 | 標示可用於室外・能上鎖或卡扣・**內部放得下捕鼠夾**（§5.1）|
| 快扣式捕鼠夾 | 6 | 50–150 元 | **不是捕鼠籠**，籠子放不進站體 |
| 6 公釐鑽頭 | 1 | 數十元 | 當量規用，不要拿原子筆比（§3.1）|
| 花生醬 | 1 罐 | 百元內 | 誘餌用，**不用藥劑** |
...
- [ ] **漢他伏原包裝密封**，收到管理中心上鎖或幼童無法取得處，標示「未使用・勿棄置戶外」。已受潮的先別丟，密封暫存（§1.1、§5.2）
- [ ] 回報：一張存放照片

### 本週

- [ ] 採購上表四項
- [ ] **建築物開口普查**（§3.2）：用 6 公釐鑽頭量，塞得進去就記下來並拍照
  - 建築物後門門底縫（最優先）
  - 其他出入口、機房門、防火門的門底縫與門框縫
  - 地下室通風百葉、機房開口
  - 管線穿牆套管孔洞（尤其後加裝的線路）
  - 排水溝蓋間距與破損處
  - 一至三樓開窗、陽台、排氣口
- [ ] **社區自身食物與水源查核**（§2.3）：環保室桶蓋與落地情形、有無人餵流浪貓或野鳥（問保全與清潔）、積水與漏水、植栽落果堆肥
- [ ] 回報：兩份清單與照片交主委，由主委發包

### 下週

- [ ] **站體就位**：放在已知鼠徑上、貼牆（2026-09-11 影像：沿花園牆基橫越 → 鐵門下緣）
- [ ] **前 3–5 天預餌**：夾具不設定，只放站體與花生醬，讓牠們習慣進出（§5.3）
...
| 每週 | 總幹事 | 巡檢、清理、重設、填監測表（§7）；調閱後門監視器（§7.0）|
| 每月 | 總幹事 | 於管委會議摘要回報趨勢 |

## 三件不要做的事

1. **不要放毒餌**——社區有黑貓夜間出沒，手上的漢他伏是第二代抗凝血劑（§9）
2. **不要補花園鐵門底縫**——花圃牆擋不住、楔形縫做不到 6 公釐、花園非防守線（§3.3）
3. **不要再向夯飽詢問或施壓**——但也未排除，繼續以監測累積證據（§2.5）
...
**2026-09 執行紀錄**：總幹事已前往夯飽以「周邊共同防範」的框架溝通，並請對方一併找消毒廠商協助。對方表示無從追究來處——**這個回應合理，我方當時也確實沒有指向性證據**。

**目前的定位是「未確認，但也未排除」。** 鄰近的餐飲業本來就是合理的候選之一，只是還不能當成結論。請把這兩件事分開：

| | 作法 |
|---|---|
| **不要再做的** | 不再就此事向對方詢問或施壓、不對外指名、不向主管機關陳報 |
| **要繼續做的** | 保留為候選來源之一，依 §2.4 持續累積監測資料與鼠徑方向；若對方委請廠商施作，記錄時程與後續變化 |

**「不施壓」不等於「不追查」。** 證據靠監測慢慢累積，不靠反覆詢問鄰居。

**若對方確實委請廠商施作，請問到時程並回報主委。** 我方已不投藥（見第九節），所以這不是為了兩側同步下藥，而是為了兩件事：

- **預期鼠隻動向改變**。鄰地開始防治時，族群可能往我方移動，這段期間監測數字上升是合理的，不要誤判為我方措施失效。
- **鄰地若使用毒餌，我方會出現中毒鼠屍**。屆時 §6.3 的鼠屍搜尋要加強——中毒鼠屍是黑貓二次中毒的來源，即使藥不是我們下的。
...
這個標準不是每個位置都做得到，事先分清楚可以省下白花的錢：

| 場合 | 可行性 | 說明 |
|---|---|---|
| **建築物的門**（後門、側門、機房門） | **做得到，而且是常規工程** | 門檻是平整完成面，裝金屬門底條或門檻板即可貼合。這是商業防鼠的標準作法 |
| **管線套管孔、通風百葉、牆面孔洞** | **做得到** | 不鏽鋼絲絨＋水泥、或金屬細網，本來就能封到零縫隙 |
| **室外鐵門、大門（地面傾斜或不平）** | **本案現場不可行** | 地面斜、門扇會擺動下沉、縫內會積灰與石礫。擋板裁得再準也難以全長貼合，且會隨使用再跑掉。能做到多少視現場而定，但要穩定維持 6 公釐，本案這道門辦不到 |

**這個不對稱本身就是資源該往哪放的答案**：建築物的門做得到 6 公釐，室外鐵門做不到。所以錢花在建築物上（§3.2），不要花在花園鐵門上（§3.3）。

> 補充：6 公釐是**小家鼠**的標準。若日後能確認現場只有**溝鼠**（約 2 公分），室外鐵門要達標就變得可行。但在種類未確認前，建築物這條紅線一律以 6 公釐從嚴。
...
## 3.3 花園周界：不投入封堵資源（含理由）

### 結論

**不要為了擋老鼠而花錢整修花園周界，包括外圍鐵門的門底縫。**

### 周界的實際構造

花園柵欄並非落地，底下是約 **60–70 公分高的花圃牆**；柵欄本身有孔洞。唯一的地面層開口是巷子側外圍鐵門的門底楔形縫，2026-09-11 監視器實拍鼠隻由此進出。

乍看之下「地面只有一個洞，補起來就好」，但這個推論不成立：

| 理由 | 說明 |
|---|---|
| **60–70 公分擋不住鼠** | 溝鼠的垂直跳躍能力已接近此高度，屋頂鼠更是輕易攀爬；何況花圃有土壤與植栽，本身就是現成踏腳處 |
| **花園通行量不是我們的指標** | 目標是守住建築（見「本案的目標是什麼」）。就算花園進出次數減半，對室內零鼠蹤這個 KPI 沒有直接貢獻 |
| **補了會讓我們失去現有的監測視野** | 後門監視器正好涵蓋鐵門這條路徑。若把地面開口封死、逼牠們改從別處翻越花圃牆，我們就再也拍不到牠們——**等於花錢買到「看不見」** |
| **成本不低** | 斜度楔形縫需實測、裁切、加焊，不是買條門底條就好 |
| **而且做不到標準** | 地面斜、門扇會擺動下沉、縫內積灰石礫，難以全長貼合且會隨使用再跑掉（見 §3.1 可行性表）。**花錢買一個達不到的規格是最糟的支出** |

第三項尤其值得留意：**在還沒守住建築之前，能持續看見對手的動線是有價值的。**

### 那鐵門到底要不要處理

**不為防鼠而做。** 但若日後鐵門因維修、更換或美觀等其他原因要施工，請順便一併處理，屆時的技術重點記錄如下，不必重新研究：

- **這是楔形縫**（地面斜、門下緣平直），且**整條都過大**——老鼠不必走到最寬處，偏窄段即可通過。故須**整條封閉**，擋板深度沿門寬漸變，不可用等寬料。
- ❌ **一般門底條無效**——假設地面與門下緣平行，裝上只貼合窄端，寬端仍留開口。
- ❌ **橡膠條與毛刷條不可用**——會被啃穿或直接鑽過。
- ❌ **不要用水泥墊平地面**——斜度多半為排水而設，填平會積水，而積水本身就是老鼠要的水源。
- ✅ **建議作法**：依實測斜度裁切鍍鋅或不鏽鋼板，加焊或鎖固於門扇下緣，板隨門移動、不影響排水。
- **驗收**：沿整條門縫每約 30 公分量一次（含兩端與中段），每一點都須小於 6 公釐。
- ⚠️ 後門是逃生動線，任何措施不得妨礙門的正常開闔與人員通行，不可上鎖固定死。

### 鐵門沒關的問題：照樣要處理，但不是當防鼠工程

這支監視器原本就是為了查核**住戶進出後有沒有把鐵門關好**而裝的。

鐵門長時間敞開本身就是**門禁與安全問題**，該依門禁管理辦法處理（提醒、自閉器），**不必也不應該包裝成防鼠措施**——如上所述，關上門也擋不住牠們翻花圃牆。兩件事分開講，理由才站得住。

### 2026-09-11 監視器影像的可用資訊

影像除了佐證現場觀察，另提供兩項：

1. **白天活動**（14:59）。鼠類本為夜行性，白天在開放區域活動**可能**反映族群密度較高（競爭把部分個體推到日間）或環境長期無干擾。**單次影像還不足以斷定，但值得留意**——若後續影像顯示日間出現轉為常態，那就是族群壓力的訊號。
2. **明確的鼠徑**：沿牆基橫越花園 → 鐵門下緣。**捕鼠夾站體請直接放在這條路徑上貼牆處**（見 §6.1）。

### 配套：不要在花圃牆面提供攀爬輔助

花圃牆雖然擋不住決心通過的老鼠，但也沒必要自己降低它：

- 牆面與牆頂**不要堆放雜物、盆器、棧板**。
- **花圃植栽不要垂到牆外側**，也不要讓藤蔓攀附牆面，併入綠石園藝例行修剪。

這幾項是**零成本的順手動作**，與上面「不花錢做周界工程」並不衝突。
...
2. **可鎖或卡扣**，幼童無法徒手打開
3. **內部有固定裝置**（餌棒、卡榫或固定孔）——用來把夾具固定住，避免被拖動；日後若改放監測塊也用得到
4. **內部空間可容納機械式捕鼠夾**——**本案這一項是必要條件，不是備用選項**（見第九節），選購前請先確認站體標示可容納捕鼠夾

**怎麼確定放得下**：先決定要用的捕鼠夾型號，**量好夾具的長寬**，再對照站體標示的內部尺寸或詢問賣家。市售有不少窄版款式是設計給餌塊的，放不進夾子——**買之前對一次尺寸，比買錯重買省事**。
...
3. **老鼠死亡前數日仍會持續取食**——中毒但尚未死亡的老鼠行動遲緩、警覺下降，**更容易被貓捕食**，因而提高二次中毒風險。這使得「用防拆站體擋住貓直接接觸毒餌」並不足以防止二次中毒。
...
- 前提是**確認貓已不再出沒**，且經主委同意。
- 屆時須**核對包裝**：品名、有效成分、環境用藥許可證字號，並完全依標示的使用方法、劑量與適用場所施用。
- **若領到的不是抗凝血劑，中毒症狀、解毒方式與死亡時程都不同**，§6.2 的標示與巡檢方式都要重寫，請先回報主委。

## 5.3 捕鼠夾的設置與巡視

### 設置

- **每個站體內裝一至二個快扣式捕鼠夾**，**依原廠標示的方向安裝**（夾口朝向站體開口，老鼠沿站內通道前進時觸發）。
- **誘餌用食物即可，不需藥劑**——花生醬效果好且黏稠，老鼠無法整塊叼走；花生、培根等高油脂高蛋白食物也可以。**量少即可。**
- **設置與拆卸時戴手套**，並小心夾具彈力。
- 站體須**固定於地面並上鎖或卡扣**，避免被搬動或幼童開啟。

### 先預餌，不要一開始就設定

老鼠對環境中新出現的物體會迴避數天到一兩週（新警戒性）。**建議前三到五天先把站體與夾具放好、夾具不設定（不上扣），只放誘餌**，讓牠們習慣進出取食；等誘餌開始被吃，再把夾具設定起來。

直接設定通常的結果是好幾天毫無動靜，然後被誤判為「沒有老鼠」。

### 巡視頻率：每日目視、每週記錄

| 頻率 | 誰 | 做什麼 |
|---|---|---|
| **每日** | 清潔或保全**順道目視** | 看夾具有沒有觸發、有沒有捕獲物。發現即通報總幹事 |
| **每週** | 總幹事 | 正式巡檢、清理、重新設置、填寫監測表 |

**為什麼要每日**：捕獲物在戶外放置過久會產生異味與蒼蠅，也讓該站失效；夾具偶爾會誤觸發或只造成受傷未死，及早處理對動物與環境都比較好。清潔與保全本來就每天巡場，順道看一眼即可，不增加額外班次。

### 捕獲物處理

- 戴手套，以雙層塑膠袋密封後丟一般垃圾，接觸處消毒。
...
- **貼牆擺放，放在已知鼠徑上**。2026-09-11 影像顯示的路徑是「沿花園牆基橫越 → 鐵門下緣」，**第一個站體就放在這條路徑貼牆處**。
- 老鼠本來就不走空曠處，所以擺對位置同時解決效果與美觀——放在牆邊灌木或設備後方，住戶不會注意到。
...
捕鼠夾的捕獲物是當場可見、可立即移除的（處理方式見 §5.3），這正是它相對毒餌的一大優點：不會有中毒的老鼠跑到別處才死、再被貓吃掉。

但**環境中仍可能出現來源不明的鼠屍**——自然死亡，或鄰地投藥造成的中毒鼠。這類鼠屍同樣要處理：

- **每週巡檢時一併搜尋**花園、灌木下、設備後方、牆角，不要等住戶聞到異味才處理。
- 處理時**戴手套**，以雙層塑膠袋密封後丟一般垃圾，接觸處消毒。
- **來源不明的鼠屍要特別留意**——若鄰地使用毒餌，中毒鼠屍就是黑貓二次中毒的來源，即使藥不是我們下的。
...
## 7.0 先用既有的監視器，它比人工巡檢更勤勞

**後門花園朝鐵門方向的監視器已經涵蓋主要鼠徑**（2026-09-11 即由它拍到）。這是現成、零成本、24 小時不間斷的監測工具，**請優先利用，不要只靠每週一次的人工巡檢**。

作法建議（挑做得到的就好，不必全做）：

- **請保全在交接時順帶留意**該鏡頭有無鼠隻通過，發現就記錄時間，寫進監測表的備註欄。
- 總幹事**每週調閱一次**，重點看夜間時段與鐵門附近。
- **發現時請從主機匯出原始片段存檔**（USB 或主機的匯出功能），不要用手機翻拍螢幕——翻拍會損失大半資訊，原始檔才看得出品種與行為細節。**若不熟悉匯出操作，請洽監視系統廠商協助一次並記下步驟。**
- 影像紀錄的價值在於**趨勢**：每週出現幾次、什麼時段、走哪條路徑。這比單次目擊有用得多。

> 這支鏡頭原本是為了查核住戶關門而裝的，現在多了第二個用途，不必另外花錢。

> **監測點不等於誘餌站。** 誘餌站兩三個就夠（放在鼠徑上），其餘監測點是**純目視點**——不放餌，只固定去看有沒有鼠糞、啃咬痕與洞口活動。目視點不花錢，可以多設幾個，涵蓋面比只看誘餌站廣得多。

| 日期 | 監測點 | 新鮮鼠糞 | 啃咬痕 | 洞口活動 | 夾具觸發／捕獲數 | 發現鼠屍 | 備註 |
|---|---|---|---|---|---|---|---|
|  | 1. |  |  |  |  |  |  |
|  | 2. |  |  |  |  |  |  |
|  | 3. |  |  |  |  |  |  |
...
**① 不要只看捕獲數。** 捕獲數下降不一定代表族群減少，也可能是：

- **新警戒性**——老鼠對新出現的物體會迴避數天到一兩週，剛擺站時沒有捕獲是正常的（所以要先預餌，見 §5.3）
- 夾具位置放錯（擺在空曠處，不在鼠徑上）
- 站體被移動或被雜物擋住
- 誘餌乾掉或被螞蟻佔據
...
**② 捕獲數高代表族群還在**，不是「措施有效」。要看到捕獲數**逐週下降**，同時其他三項也下降。

**③ 完全沒有動靜**時，先檢查是不是還在新警戒期、誘餌狀態如何、位置是否放錯，不要直接判定沒有老鼠。**這是最常見的誤判。**
...
這樣做的理由：目標是壓低壓力、守住建築，不是清空花園（花園清不空）。既然如此，沒有必要長期維持捕捉作業的人力負擔——監測塊採購成本低、沒有捕獲物要處理，巡檢也只要每月一次，照樣看得出活動強度。
