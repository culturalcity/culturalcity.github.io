...
### 3.4.1 保固起算點：依買賣契約而非「直覺」

保固到底從哪天算？**沒有唯一標準答案，得回去看契約**。一般買賣契約會寫明三種起算之一：

| 契約常見起算 | 涵義 | 對主委的意義 |
|---|---|---|
| **自交屋日起算** | 每戶交屋日不同 | 住戶端瑕疵保固（馬桶、廚具等戶內項目）|
| **自使用執照核發日起算** | 統一一個日期，但比實際使用早 | 比較少見 |
| **自公共設施點交日起算** | 管委會代表全體區權人的公設保固 | **這是主委最常用到的條款** |

**閱大安公設保固實際起算**：

- **使用執照**：民國 110 年（2021）10 月核發
...
- **公設點交完成**：**民國 112 年（2023）2 月 6 日**（第二屆第一次區權會紀錄載明）
- **5 年保固起算**：2023-02-06
- **5 年保固屆滿**：**2028-02-06**

> 📌 這份起算日是查證自會議紀錄的事實，**不需要另作確認**。但**買賣契約**裡寫的住戶端保固起算日（每戶交屋日）可能與此不同——若涉及住戶端瑕疵爭議要回去看個別契約。

### 3.4.2 保固期長短：不是一律 5 年

不同項目有不同保固期，**主結構長、外殼中、設備短**：

| 項目類別 | 法定／契約常見保固 | 法源／來源 |
|---|---|---|
| **主要結構**（樑、柱、樓板、剪力牆）| **15 年**（部分契約 10 年）| 民法 + 買賣契約 |
| **外殼**（外牆、屋頂、防水層）| **5 年** | 買賣契約常見 |
| **機電設備**（泵、馬達、UPS）| **1-2 年**（依設備合約）| 設備供應商保固 |
| **電梯** | **1-2 年**（廠商保固）| 三菱保固 |
| **消防設備** | **1-2 年** | 設備供應商 |
| **弱電設備**（監視、門禁）| **1-3 年** | 設備供應商 |
| **裝修飾材**（地坪、油漆）| **1 年** | 契約常見 |

**閱大安主委該注意**：

- **2026-2027**：5 年外殼／部分設備保固陸續屆滿——**剩 6-12 個月**
- **2031-2036**：10 年部分結構保固陸續屆滿
- **2036-2037**：15 年主結構保固屆滿
...
**核心原則**：**舉證責任在建商**——建商主張「地震造成」需要拿出證據，否則應視為原始施工瑕疵屬保固範圍。

```mermaid
flowchart TD
  D[爭議發生：建商主張地震造成]
  D --> Q1{1. 這個問題在<br/>0403 地震之前<br/>就有紀錄嗎？}
  Q1 -->|是| W1[✅ 地震前就有<br/>建商必修]
  Q1 -->|否| Q2{2. 漏水點是<br/>原始施工縫<br/>還是新裂縫？}
  Q2 -->|沿原施工縫| W2[✅ 原始瑕疵<br/>建商必修]
  Q2 -->|新出現裂縫| Q3{3. 建商能否提出<br/>地震受力分析證據？}
  Q3 -->|無法提出| W3[✅ 視為原始瑕疵<br/>建商必修]
  Q3 -->|能提出| N[⚠️ 進入第三方鑑定]
  N --> NA[請結構技師公會<br/>或營建研究院鑑定]
  NA -->|地震造成| L1[❌ 不在保固]
  NA -->|施工瑕疵| L2[✅ 建商必修]
  NA -->|混合因素| L3[協商分攤費用]
```
...
接任主委請**從現在開始**累積這些證據（地震越早、證據越完整越有利）：

1. **點交清冊 + 點交照片**（黃金證據）——「**這個東西點交時就是這樣**」是最強的反駁
2. **點交至今所有報修紀錄**——「**這個漏水點 2022 年 5 月住戶就反映過**」直接打死「地震後才出現」的說法
3. **地震前的全棟照片**（理想）——譬如年度巡查照片
4. **地震發生當天的損害紀錄**——立即拍照、書面記錄
5. **0403 後的全棟複勘照片**——把「地震前 vs 地震後」狀態區分開
...
4. **拍照存證的紀律從今天開始**——點交清冊、年度照片、報修前後對照
...
**對主委的意義**：**任何剪力牆絕對不能打掉**。921 地震後台灣建築規範大幅強化，「韌性設計」是 2003 年後的新建物標準。閱大安建照核發於民國 104 年（西元 2015 年）、使照於民國 110 年（西元 2021 年）10 月——屬新規範後的建物，理論上韌性設計到位。
...
**待確認**：閱大安屬 RC、SRC 或鋼骨結構？可從使用執照或建照圖說查到。

> ⚠️ **TODO（接任主委請補充）**：跟總幹事查使用執照後在此補充本社區結構類型，影響日後維護重點

從外觀推測：閱大安為 15 樓住宅大樓，**最有可能是 RC 或 SRC**（鋼骨在 25 樓以下少見）。
