---
layout: handbook.njk
permalink: /admin/chair-handbook/index.html
title: 閱大安主委手冊
docTitle: 主委手冊
...
這份文件是**閱大安主委**的整體 reference——涵蓋**工程實務、行政法規、判斷工具**三大維度。跟《總幹事手冊》（執行層 SOP）配對使用，**兩本書合起來覆蓋社區管理的全部運作**。
...
- **第一次當主委**：建議從頭讀到尾，4-8 小時（不必一次讀完）。讀完不必記住細節，**先建立 mental model**
- **遇到具體議題**：直接跳對應章節（Ch 4-10 工程、Ch 11-16 行政法規）
- **跟廠商開會前**：掃一下該系統的「§X.末末 不必懂但要會問」清單
- **跨任交接時**：把這份文件直接交給下任主委，比口述快十倍

文件刻意寫得**淺**——讓你能在通勤、午休、洗澡前一段段啃完。**寫得深的執行細節在《閱大安總幹事手冊》**（`/admin/handbook/`），那邊有具體 SOP、廠商電話、合約附註，是「執行層」的文件。本份是「**決策層**」的文件。

> 兩份文件的差異：總幹事手冊回答「**怎麼做**」，本份回答「**這是什麼、依據什麼、什麼時候我該介入、怎麼判斷**」。
...
- **《主委對外窗口地圖》**（`/admin/chair-gov-map/`）：跟臺北市政府 15+ 個對接科的快查清單。本書 cross-link 過去而不重複收錄
